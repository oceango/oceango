...
func GetDb() *gorm.DB {
	...
	args := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True", dataSource.Username, dataSource.Password, dataSource.Host, dataSource.Port, dataSource.Database)
